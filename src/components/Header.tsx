      ...
      if (data.user) {
        ...
        if (profile) {
          ...
          const { data: cr } = await supabase.from('user_custom_roles')
            .select('id, label, icon, color, description, has_glow, has_pulse').eq('user_id', data.user.id);
    ...
    const profileChannel = supabase.channel('my_profile_sync')
      .on('postgres_changes',
        ...
        async () => {
          ...
          if (p) {
            const { data: cr } = await supabase.from('user_custom_roles')
              .select('id, label, icon, color, description, has_glow, has_pulse').eq('user_id', user.id);
