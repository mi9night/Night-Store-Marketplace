...
import {
  ArrowLeft, ShoppingCart, Zap, Heart, Shield, Star, CheckCircle2, AlertCircle,
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      if (data?.ok) {
        ...
        setBuyResult({ type: 'err', text: human[err] || err });
      }
    } catch (e: any) {
      setBuyResult({ type: 'err', text: e.message || 'Ошибка покупки' });
    } finally {
      setBuying(false);
    }
  };
          ...
          >
            ...
            <div className="space-y-3 mb-4">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold bg-accent hover:bg-accent-hover text-white disabled:opacity-50"
              >
                <Zap size={18} />
              ...
              {buyResult && (
                <div className={`text-sm p-3 rounded-lg flex items-start gap-2 ${
                  buyResult.type === 'ok' ? 'bg-success/10 text-success' : 'bg-error/10 text-error'
                }`}>
                  {buyResult.type === 'ok' ? <CheckCircle2 size={16} className="flex-shrink-0 mt-0.5" /> : <AlertCircle size={16} className="flex-shrink-0 mt-0.5" />}
